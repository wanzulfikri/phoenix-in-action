defmodule Auction do
  ...
  application, these functions are provides. Any interaction you need to do with the database can be done from within
  these functions. See an individual function's documentation for more information and usage examples
  (like `Auction.get_user_by_username_and_password/2`)

  """
  alias Auction.{Bid, Repo, User, Item, Password}
  import Ecto.Query
  @repo Repo
  ...
  * `false`: No `Auction.User` could be found with the provided `username`
  ...
  def insert_bid(params) do
    %Bid{} |> Bid.changeset(params) |> @repo.insert()
  end
